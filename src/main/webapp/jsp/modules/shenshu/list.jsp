            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">申请表管理</li>
                        <li class="breadcrumb-item active">申请表列表</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">申请表列表</h3>
                        <div class="table-responsive mb-3">
                            ...
                            <table id="tableId" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th onclick="sort('xsTypes')">申请人</th>
                                    <th onclick="sort('createTime')">申请时间</th>
                                    <th onclick="sort('noticeContent')">申请内容</th>
                                    <th>操作</th>
                                </tr>
                                </thead>
                                <tbody>
...
<script>
    ...
    function setDataRow(item, number) {
        ...
        var jlbnameCell = document.createElement('td');
        jlbnameCell.innerHTML = item.jlbname;
        row.appendChild(jlbnameCell);

        var xsnameCell = document.createElement('td');
        xsnameCell.innerHTML = item.xsname;
        row.appendChild(xsnameCell);
        ...
        if(item.temporary == null){
            ...
            var detailAttr = "consent(" + item.id + ')';
            detailBtn.setAttribute("type", "button");
            detailBtn.setAttribute("class", "btn btn-info btn-sm 同意");
        ...
        if(item.temporary != null){
            var detailBtn = document.createElement('button');
            var detailAttr = "consentTransfer(" + item.id + ')';
            detailBtn.setAttribute("type", "button");
            detailBtn.setAttribute("class", "btn btn-info btn-sm 同意");
            detailBtn.setAttribute("onclick", detailAttr);
            detailBtn.innerHTML = "同意转会"
            btnGroup.appendChild(detailBtn)
        }
    ...
    // 删除
    function consentTransfer(id) {
        var mymessage = confirm("真的要同意转会吗？，本次操作是不可撤回的");
        if (mymessage == true) {
            paramArray = id;
            httpJson("xuanshou/consentTransfer", "POST", paramArray, (res) => {
                if(res.code == 0
        )
            {
                getDataList();
    ...
    // 删除
    function consent(id) {
        var mymessage = confirm("真的要同意吗？，本次操作是不可撤回的");
        if (mymessage == true) {
            paramArray = id;
            httpJson("xuanshou/consent", "POST", paramArray, (res) => {
                if(res.code == 0
        )
            {
                getDataList();
    ...
    function remove(id) {
        ...
        if (mymessage == true) {
            ...
            httpJson("shenshu/delete", "POST", paramArray, (res) => {
                if(res.code == 0
        )
            {
                getDataList();
    ...
    //查询当前页面下所有列表
        ...
        function xsTypesSelect() {
            //填充下拉框选项
            http("xuanshou/page?page=1&limit=100&sort=&order=&dicCode=xs_types", "GET", {}, (res) => {
                if(res.code == 0){
    ...
    function crossTable(id, crossTableName) {
        ...
        window.sessionStorage.setItem('crossTableName', "shenshu");
